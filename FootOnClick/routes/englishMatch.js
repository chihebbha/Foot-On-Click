...
var url='http://api.football-data.org/v1/soccerseasons/398/fixtures?timeFrame=n10';
var entete={headers:{
...
router.get('/nextMatch', function(req, res, next) {
    request(entete, function (err, data){
        ...
        res.render('nextMatchEnglish.twig',{data:match});
    });
});
...
router.get('/pastMatch', function(req, res, next) {
    ...
    request({headers:{
        ...
        'uri':'http://api.football-data.org/v1/soccerseasons/398/fixtures?timeFrame=p10'}, function (err, data){
        var  flux=JSON.stringify(JSON.parse(data.body));
        match=JSON.parse(flux).fixtures;
        ...
        res.render('apiEnglish.twig',{data:match});
    });
});
//----------------------------------------------------------------
router.get('/morePast', function(req, res, next) {
    ...
    request({headers:{
        ...
        'uri':'http://api.football-data.org/v1/soccerseasons/398/fixtures?timeFrame=p30'}, function (err, data){
        var  flux=JSON.stringify(JSON.parse(data.body));
        match=JSON.parse(flux).fixtures;
        for(var i=0;i<match.length;i++){
            var dateee=match[i].date.toString();
...
router.get('/moreNext', function(req, res, next) {
    request({headers:{
        ...
        'uri':'http://api.football-data.org/v1/soccerseasons/398/fixtures?timeFrame=n30'}, function (err, data){
        var  flux=JSON.stringify(JSON.parse(data.body));
        match=JSON.parse(flux).fixtures;
        mLab.insertDocuments({
            database: 'footonclick',
